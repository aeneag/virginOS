...
bool_t ret_msadsc_vadrandsz(machbstart_t *mbsp, msadsc_t **retmasvp, u64_t *retmasnr)
{
	...
	for (u64_t i = 0; i < mbsp->mb_e820exnr; i++)
	{
		if (PMR_T_OSAPUSERRAM == pmagep[i].pmr_type)
		{
			...
			msadnr += (pmagep[i].pmr_lsize >> 12);//这个计算数量真的绝了
		}
	}
...
//处理单个msadsc 
void write_one_msadsc(msadsc_t *msad, u64_t phyadr)
{
	...
	msad->md_phyadrs.paf_padrs = tmp->paf_padrs;
	return;
}
...
void init_msadsc()
{
	u64_t coremdnr = 0, msadscnr = 0;//msadscnr :msadsc_t 数量
	msadsc_t *msadscvp = NULL;//get msadsc_t struct start address
	machbstart_t *mbsp = &kmachbsp; // hal层全局机器信息结构体
	//计算msadsc_t结构数组的开始地址和数组元素个数

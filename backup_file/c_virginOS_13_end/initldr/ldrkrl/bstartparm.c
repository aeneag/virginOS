...
void init_bstartparm()
{
    ...
    machbstart_t_init(mbsp);                        //初始化这个结构体，清零，并设置一个标志
    init_chkcpu(mbsp);                              //检查cpu
    init_mem(mbsp);                                 //获取内存布局
    ...
    init_meme820(mbsp);                             //将内存信息复制到kernel后面，并重新更新信息结构体
    init_bstartpages(mbsp);                         //建立MMU页表
    init_graph(mbsp);                               //
    return;
}

...
*  返回 
**************************************************************************************************/
void show_virginOS_start(machbstart_t *mbsp,char_t *fname){
    u32_t retadr = 0, sz = 0;
    get_file_rpadrandsz(fname, mbsp, &retadr, &sz);
        if (0 == retadr)
    {
...
void show_all(machbstart_t *mbsp){
    char show_name[15] = {"AenV9os018.bmp"};
    for(int i = 5 ; i < 253; ++i){
        mer(show_name,i);
        show_virginOS_start(mbsp,show_name);
    }
    die(0x100);

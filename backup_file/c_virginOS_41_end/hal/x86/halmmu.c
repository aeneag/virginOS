...
sdirearr_t* mmu_transform_sdire(mmudsc_t* mmulocked, tdirearr_t* tdirearr, adr_t vadrs, u64_t flags, msadsc_t** outmsa)
{
	...
	tdire = tdirearr->tde_arr[tindex];
	if(sdire_is_have(&tdire) == TRUE)//判断页目录指针是否存在
	{
		sdirearr = tdire_ret_sdirearr(&tdire);//获取 页目录指针 虚拟地址
		*outmsa = NULL;
		return sdirearr;
	}

	msa = mmu_new_sdirearr(mmulocked);//不存在 创建新的
	if(NULL == msa)
	{
		*outmsa = NULL;
		return NULL;
...
* Aen
* https://aeneag.xyz
* description:mmu 映射 核心 函数
*   注：映射和卸载映射   从页目录指针  页目录  页表项
*            
*  返回 null
**************************************************************************************************/

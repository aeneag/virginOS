...
bool_t merlove_scan_continumsadsc(memarea_t *mareap, msadsc_t *fmstat, uint_t *fntmsanr, uint_t fmsanr,
								 //      区          msadsc_t 开始地址   这次循环用到第几个msadsc_t了    msadsc_t 数量
								  msadsc_t **retmsastatp, msadsc_t **retmsaendp, uint_t *retfmnr)
								  //      当前连续msadsc_t 开始地址    结束地址     数量
{
	...
	{   							// 是个区的不
		if (msastat[tmidx].md_indxflgs.mf_marty == mdfp->mf_marty &&
			0 == msastat[tmidx].md_indxflgs.mf_uindx &&//分配计数，分配过没
			MF_MOCTY_FREE == msastat[tmidx].md_indxflgs.mf_mocty && // 是空闲未分配
			PAF_NO_ALLOC == msastat[tmidx].md_phyadrs.paf_alloc)//是否分配了
		{
			//返回从这个msadsc_t结构开始到下一个非空闲、地址非连续的msadsc_t结构对应的msadsc_t结构索引号到retfindmnr变量中
...
* memarea_t *mareap, bafhlst_t *bafhp, msadsc_t *fstat, msadsc_t *fend, uint_t fmnr         
*           区            对应的结构体       页开始          页 结束        页数量
*/
bool_t continumsadsc_add_bafhlst(memarea_t *mareap, bafhlst_t *bafhp, msadsc_t *fstat, msadsc_t *fend, uint_t fmnr)
{
	...
	bafhp->af_fobjnr++;//该多少个空闲的页
	bafhp->af_mobjnr++;//该结构的页数
	mareap->ma_maxpages += fmnr;//该区的总页数
	mareap->ma_freepages += fmnr;//该区空闲页面数
	mareap->ma_allmsadscnr += fmnr;// 分配的页面数
	return TRUE;
}
...
* Aen
* https://aeneag.xyz
* memarea_t *mareap, msadsc_t *mstat, uint_t msanr
*            区          所有页首地址    所有页的数量
*
*/
